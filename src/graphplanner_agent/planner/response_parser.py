...
def _extract_json(text: str):
    ...
    for idx, ch in enumerate(text):
        ...
        if trailing:
            raise ValueError(f"Extra data after planner action: {trailing[:120]}")
        return value
    raise ValueError("planner response did not contain a JSON action")


def _coerce_single_action_payload(payload):
    if isinstance(payload, list):
        if not payload:
            raise ValueError("planner response contained an empty JSON action array; emit exactly one JSON object")
        first = payload[0]
        if not isinstance(first, dict):
            raise ValueError("planner response JSON array did not start with an action object")
        return first
    if not isinstance(payload, dict):
        raise ValueError("planner response JSON value must be one action object")
    return payload
...
def parse_planner_response(text: str) -> ParsedPlannerResponse:
    ...
    payload = _coerce_single_action_payload(_extract_json(formal))
    return ParsedPlannerResponse(
        action=PlannerAction.from_obj(payload),
        visible_thinking=visible,
        formal_text=json.dumps(payload, ensure_ascii=False, sort_keys=True),
    )
