...
# 配置 runner 环境变量
export QUEUE_ROOT="${QUEUE_ROOT:-$HOME/gp_queue}"
export RUNNER_ID="${RUNNER_ID}"
export SHARE_ROOT="${SHARE_ROOT:-$HOME}"
export APPTAINER_BIN="singularity"
export GRAPHPLANNER_REMOTE_REPO="${GRAPHPLANNER_REMOTE_REPO:-${GP_REMOTE_REPO:-$HOME/MARL_CGM}}"

cd "$GRAPHPLANNER_REMOTE_REPO"
