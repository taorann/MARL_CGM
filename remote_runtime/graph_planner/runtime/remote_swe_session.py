...
class RemoteSweSession:
    ...
    * Forward start/exec/stop JSON requests to swe_proxy.py with GP_NUM_RUNNERS set.
    ...
    def _ssh_base_cmd(self) -> list[str]:
        ...
        insecure = os.environ.get("GP_REMOTE_SWE_INSECURE_SSH", "").strip().lower() in {
            "1",
            "true",
            "yes",
        }
        ...
        extra_args = (os.environ.get("GP_REMOTE_SWE_SSH_ARGS") or "").strip()
        if extra_args:
            try:
                cmd.extend(shlex.split(extra_args))
            except Exception:
                _dbg(f"ignoring invalid GP_REMOTE_SWE_SSH_ARGS={extra_args!r}")
        return cmd
    ...
    def _call_proxy(self, payload: Dict[str, Any], timeout: Optional[float] = None) -> Dict[str, Any]:
        ...
        # NOTE: build_repo_graph can return multi-MB base64 payloads; if stdout is not
        # drained continuously, the remote proxy can block on a full pipe buffer.
        while True:
            watch_fds = []
            if stdout_open:
                watch_fds.append(proc.stdout)
            if stderr_open:
                watch_fds.append(proc.stderr)

            try:
                r, _, _ = select.select(watch_fds, [], [], 0.2) if watch_fds else ([], [], [])
            except Exception:
                r = []
            ...
            rc = proc.poll()
            if rc is not None and (not stdout_open) and (not stderr_open):
                break
        ...
        # Drain any remaining bytes (best effort; normally handled in-loop).
        try:
            if stdout_open:
