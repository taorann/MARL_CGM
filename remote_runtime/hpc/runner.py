...
def _apptainer_base_cmd(*, include_writable: bool) -> list[str]:
    # Keep only the binary here. `--writable-tmpfs` is a subcommand option
    # and must be appended after `exec` / `instance start`.
    return [APPTAINER_BIN]
    ...
    try:
        ...
        if req.type == "exec":
            ...
            resp = QueueResponse(
                ...
                returncode=2,
                stdout="",
                stderr=f"unsupported request type: {req.type}",
                runtime_sec=0.0,
                error=f"unsupported_request_type:{req.type}",
            )
...
def handle_exec(req: QueueRequest) -> QueueResponse:
    ...
    cmd = _apptainer_base_cmd(include_writable=True)
    cmd.extend(["exec", "--cleanenv", "--bind", f"{SHARE_ROOT}:/mnt/share"])
    if _env_flag("RUNNER_APPTAINER_WRITABLE_TMPFS", True):
...
def handle_instance_start(req: QueueRequest) -> QueueResponse:
    ...
    if _env_flag("RUNNER_APPTAINER_WRITABLE_TMPFS", True):
        cmd.append("--writable-tmpfs")
    cmd.extend([req.sif_path, INSTANCE_NAME, "bash", "--noprofile", "--norc", "-c", keepalive_cmd])
