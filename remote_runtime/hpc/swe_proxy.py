...
def _instance_roundtrip(
    ...
    *,
    ...
    if op_type == "instance_start":
        # New instances should wait for an idle runner.
        runner_id = aq.choose_runner(run_id, image=image)  # type: ignore[attr-defined]
    else:
        # Existing-instance operations must be routed to the runner that already
        # owns this run_id. Falling back to an idle runner can execute against
        # no container, or worse, against a different task's bound runner.
        wait_sec = float(os.environ.get("GP_RUNNER_EXISTING_WAIT_SEC", "15") or "15")
        runner_id = aq.choose_runner_for_existing(run_id, wait_sec=wait_sec)  # type: ignore[attr-defined]
    ...
    if cmd is None:
        ...
        cmd_list = ["bash", "--noprofile", "--norc", "-c", cmd]
    ...
    try:
        ...
        if op in {"start", "exec", "stop", "build_graph", "build_repo_graph"}:
            ...
            if op == "start":
                ...
                repo = str(_canon_pwd(payload.get("repo")) or "/testbed")
                py = "PYTHONPATH=$PYTHONPATH:/mnt/share/MARL_CGM:/gp"
                # Repo graph should stay structural and lightweight for transport over SSH/stdout.
                # Large embedded snippet payloads can make big repos (e.g. astropy) very slow
                # to build/compress/transfer and may hit timeout ceilings.
                embed_snippets = os.environ.get("GP_REPO_GRAPH_EMBED_SNIPPETS", "0").strip().lower()
                if embed_snippets in {"1", "true", "yes", "y"}:
                    graph_env = "GP_EMBED_REPO_SNIPPETS=1"
                else:
                    graph_env = (
                        "GP_EMBED_REPO_SNIPPETS=0 "
                        "GP_MAX_FILE_SNIPPET_LINES=0 "
                        "GP_MAX_DEF_SNIPPET_LINES=0 "
                        "GP_MAX_CLASS_SNIPPET_LINES=0"
                    )
                graph_frontend = str(os.environ.get("GP_REPO_GRAPH_FRONTEND", "treesitter") or "treesitter").strip().lower()
                if graph_frontend in {"tree-sitter", "tree_sitter", "ts"}:
                    graph_frontend = "treesitter"
                if graph_frontend in {"ast", "auto", "treesitter"}:
                    graph_env = f"{graph_env} GP_GRAPH_FRONTEND={graph_frontend}"
                build_cmd = (
                    f"{py} {graph_env} python -m graph_planner.tools.swe_build_graph "
                ...
                try:
                    ...
                    if "Cannot resolve runner" not in str(exc):
                        raise
                    # Stop is a cleanup operation. If the run_id is no longer
                    # visible in heartbeats, fan out a best-effort stop instead
                    # of surfacing an infra error to the task.
                    results = []
                    for rid in (aq._alive_runner_ids() or list(range(max(int(num_runners), 1)))):  # type: ignore[attr-defined]
                        try:
                            r = aq.stop_instance_on_runner(  # type: ignore[attr-defined]
                                runner_id=int(rid),
                                run_id=run_id,
                            ...
                            results.append(
                                {
                                    ...
                                    "ok": False,
                                    "returncode": 1,
                                    "stderr": f"{type(stop_exc).__name__}: {stop_exc}",
                                }
                            )
                    print(
                        json.dumps(
                            {
                                ...
                                "returncode": 0,
                                "stdout": "",
                                "stderr": f"fallback stop after unresolved run_id: {exc}",
                                "runtime_sec": 0.0,
                                "error": None,
                                "fallback_stop_results": results,
                            },
                            ensure_ascii=False,
                        )
                    )
        ...
        res = aq.exec(
            ...
            cmd=["bash", "--noprofile", "--norc", "-c", str(cmd)],
            cwd=cwd,
            env=env,
            timeout_sec=timeout,
        )
