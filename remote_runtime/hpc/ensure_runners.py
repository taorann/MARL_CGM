...
def _squeue_jobs(user: str) -> List[SlurmJob]:
    # %T gives long state names, %M gives elapsed time. Use a delimiter because
    # Slurm state names and future fields are easier to parse robustly this way.
    out = subprocess.check_output(["squeue", "-u", user, "-h", "-o", "%i|%j|%T|%M"], text=True)
    jobs: List[SlurmJob] = []
    for line in out.splitlines():
        ...
        jobs.append(SlurmJob(job_id=job_id, name=name, state=state, elapsed=elapsed))
    return jobs
...
def _heartbeat_age_sec(queue_root: Path, runner_id: int) -> Optional[float]:
    hb_path = queue_root / f"runner-{runner_id}" / "heartbeat.json"
...
def _prune_stale_running_jobs(
    keep: Dict[int, SlurmJob],
    *,
    queue_root: Path,
    heartbeat_ttl_sec: float,
    running_grace_sec: float,
) -> Dict[int, SlurmJob]:
    fresh: Dict[int, SlurmJob] = {}
    stale_job_ids: List[str] = []
    for rid, job in keep.items():
        state = (job.state or "").upper()
        if state not in {"RUNNING", "R"}:
            fresh[rid] = job
            continue
        elapsed = _elapsed_seconds(job.elapsed)
        age = _heartbeat_age_sec(queue_root, rid)
        if elapsed >= running_grace_sec and (age is None or age > heartbeat_ttl_sec):
            age_text = "missing" if age is None else f"{age:.1f}s"
            print(
                "[ensure_runners] STALE_HEARTBEAT: "
                f"rid={rid} job={job.job_id} elapsed={job.elapsed} heartbeat_age={age_text}; canceling"
            )
            stale_job_ids.append(job.job_id)
            continue
        fresh[rid] = job
    if stale_job_ids:
        subprocess.run(["scancel", *stale_job_ids], check=False)
    return fresh
...
def _submit_runner(
    ...
    *,
    ...
    export_parts = [f"RUNNER_ID={runner_id}", f"QUEUE_ROOT={queue_root}"]
    for name in (
        "SHARE_ROOT",
        "GP_SIF_DIR",
        "SIF_DIR",
        "PYTHONDONTWRITEBYTECODE",
        "GRAPHPLANNER_REMOTE_REPO",
        "GP_REMOTE_REPO",
    ):
        value = os.environ.get(name)
        if value:
            export_parts.append(f"{name}={value}")
...
def main() -> None:
    ...
    with _DirLock(_lock_path(queue_root)):
        ...
        keep = _prune_stale_running_jobs(
            keep,
            queue_root=queue_root,
            heartbeat_ttl_sec=float(os.environ.get("GP_ENSURE_RUNNER_HEARTBEAT_TTL_SEC", "300") or 300.0),
            running_grace_sec=float(os.environ.get("GP_ENSURE_RUNNER_GRACE_SEC", "60") or 60.0),
        )
