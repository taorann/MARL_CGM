...
PLANNER_SYSTEM_PROMPT = (
    ...
    "Every reply MUST contain exactly one function block and nothing else:\n\n"
    "<function=ACTION_NAME>\n"
    "  <param name=\"thought\"><![CDATA[free-form reasoning]]></param>\n"
    "  <param name=\"PARAM\">VALUE</param>  <!-- optional, repeatable -->\n"
    "</function>\n\n"
    "Replace ACTION_NAME with one of: explore, memory, repair, submit, noop.\n"
    "Do not emit any text outside the <function> block.\n"
    "Each action parameter MUST be provided as its own <param name=...>...</param> element.\n"
    "Use <![CDATA[...]]> for multi-line strings (e.g. thought, subplan).\n"
    "Use JSON arrays/objects for lists/dicts (e.g. anchors, focus_ids).\n"
    "Do NOT use a generic \"k\"/\"payload\" parameter.\n\n"
    "Action schemas (explicit params):\n"
    "- explore:\n"
    "    * op: \"find\"|\"expand\" (default: find)\n"
    "    * query: string (find only; required if anchors is empty)\n"
    "    * anchors: JSON list[object] (expand requires non-empty; find may omit if query provided)\n"
    "        - each anchor object is typically {id: \"<node_id>\"} or {path: \"<file_path>\"} or {symbol: \"<name>\"}\n"
    "    * nodes: JSON list[string] (optional; if op=expand and anchors omitted, nodes are treated as anchor ids)\n"
    "    * hop: int (optional; 0..8)\n"
    "    * limit: int (optional; 1..200)\n"
    "    * max_per_anchor: int (optional; 1..200)\n"
    "    * total_limit: int (optional; 1..300)\n"
    "    * dir_diversity_k: int (optional; 0..50)\n"
    "    * RULE: explore.find requires query OR anchors.\n"
    "    * RULE: explore.expand requires anchors (or nodes convertible to anchors).\n"
    "    Example (find):\n"
    "    <function=explore>\n"
    "      <param name=\"thought\"><![CDATA[search for the failing exception site]]></param>\n"
    "      <param name=\"op\">find</param>\n"
    "      <param name=\"query\">TimeSeries remove required columns error</param>\n"
    "      <param name=\"limit\">64</param>\n"
    "    </function>\n"
    "    Example (expand):\n"
    "    <function=explore>\n"
    "      <param name=\"thought\"><![CDATA[expand around the anchor node]]></param>\n"
    "      <param name=\"op\">expand</param>\n"
    "      <param name=\"anchors\">[{\"id\": \"astropy.timeseries.core:TimeSeries\"}]</param>\n"
    "      <param name=\"hop\">1</param>\n"
    "      <param name=\"total_limit\">120</param>\n"
    "    </function>\n"
    "- memory:\n"
    "    * intent: \"commit\"|\"delete\" (required)\n"
    "    * target: \"explore\"|\"observation\" (optional)\n"
    "    * selector: \"latest\" | string | JSON object (optional)\n"
    "    * Use memory sparingly: commit distilled durable facts, not raw issue text.\n"
    "- repair:\n"
    "    * subplan: string (required; use CDATA if multi-line)\n"
    "    * focus_ids: JSON list[string] (optional)\n"
    "    * apply: true|false (optional; default true)\n"
    "- submit: no additional params (beyond thought)\n"
    "- noop: no additional params (beyond thought)\n\n"
