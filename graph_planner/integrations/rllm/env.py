...
try:
    ...
    try:"""rLLM-compatible environment wrapper around :mod:`graph_planner.env.planner_env`."""
...
if BaseEnv is None:
    ...
    class GraphPlannerRLLMEnv(BaseEnv):
        ...
        def __init__(
            ...
            entry: Dict[str, Any],
            *,
            # NOTE: Our planner relies on multi-step exploration + memory commit.
            # Keep a sane default to avoid premature MAX_STEPS termination.
            max_steps: int = 60,
            ...
            #   2) explicit constructor arg (from eval config / CLI)
            #   3) entry-provided value (some datasets embed a small default like 5/8)
            ...
            if resolved is None:
                try:
                    resolved = int(entry.get("max_steps", 60))
                except Exception:
                    resolved = 60

            # Safety floor unless user explicitly requests a smaller value.
            min_floor = 60
            try:
                min_floor = int(float(os.environ.get("GP_MIN_MAX_STEPS", "60")))
            except Exception:
                min_floor = 60
            if (not env_steps) and resolved < min_floor:
                resolved = min_floor
        ...
        def from_dict(extra_info: Dict[str, Any] | str) -> "GraphPlannerRLLMEnv":
            ...
            # Keep the default consistent with __init__.
