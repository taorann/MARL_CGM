...
if BaseEnv is None:
    ...
    class GraphPlannerRLLMEnv(BaseEnv):
        ...
        def __init__(
            ...
            entry: Dict[str, Any],
            *,
            # NOTE: keep a sane default; dataset entries sometimes embed small
            # values (e.g., 5/8) that prematurely terminate exploration.
            max_steps: int = 60,
            ...
            # Resolve max_steps with clear precedence:
            #   1) env override (GP_MAX_STEPS / GRAPH_PLANNER_MAX_STEPS)
            #   2) constructor arg (from eval config / CLI)
            #   NOTE: we intentionally do NOT let entry['max_steps'] override
            #   the constructor arg, because many datasets embed tiny defaults.
            env_steps = os.environ.get("GP_MAX_STEPS") or os.environ.get("GRAPH_PLANNER_MAX_STEPS")
            if env_steps is not None and str(env_steps).strip() != "":
                try:
                    resolved = int(float(str(env_steps).strip()))
                except Exception:
            ...
            # Safety floor unless user explicitly overrides via env.
            if env_steps is None or str(env_steps).strip() == "":
                try:
                    floor = int(float(os.environ.get("GP_MIN_MAX_STEPS", "60")))
                except Exception:
                    floor = 60
                if resolved < floor:
                    resolved = floor
        ...
        def from_dict(extra_info: Dict[str, Any] | str) -> "GraphPlannerRLLMEnv":
            ...
            # Keep consistent with constructor default/floor.
            max_steps = int(extra_payload.get("max_steps", 60))
        ...
        def _build_issue_payload(self) -> Dict[str, Any]:
            ...
            # Propagate the step budget down to PlannerEnv so its internal
            # logic (and telemetry) aligns with the rLLM engine limit.
            try:
